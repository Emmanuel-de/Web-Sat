...
.form-input:disabled,.form-input[readonly]{opacity:.35;cursor:not-allowed}
...
@media(max-width:640px){.kpi-strip{grid-template-columns:1fr 1fr}.form-grid-2,.form-grid-3{grid-template-columns:1fr}.isr-grid{grid-template-columns:1fr}}
</style>
@endpush
...
<div class="panel-card">
 <div class="panel-header">
  ...
  <div class="table-controls">
   ...
   <a href="#form-declaracion" class="btn-new"><i class="fas fa-plus"></i> Nueva declaración</a>
  </div>
 </div>
 ...
 <div class="table-wrap">
  <table id="tabla-declaraciones">
   <thead>
    <tr>
     <th>Folio / Acuse</th><th>Tipo</th><th>Período</th><th>Ejercicio</th>
     <th>ISR Determinado</th><th>ISR Retenido</th><th>Saldo a cargo/favor</th>
     <th>Estatus</th><th>Fecha presentación</th><th style="text-align:center">Acciones</th>
    </tr>
   </thead>
   <tbody id="tbody-declaraciones">
    ...
    <tr data-tipo="Anual" data-estatus="presentada">
     <td class="td-mono">DEC-2025-0001</td><td class="td-bold">Anual</td><td>Enero – Diciembre</td><td>2024</td>
     <td class="td-bold">$18,400.00</td><td>$14,200.00</td>
    ...
    <tr data-tipo="Mensual" data-estatus="presentada">
     <td class="td-mono">DEC-2025-0024</td><td class="td-bold">Mensual</td><td>Diciembre</td><td>2025</td>
     <td class="td-bold">$3,210.00</td><td>$2,100.00</td>
    ...
    <tr data-tipo="Mensual" data-estatus="presentada">
     <td class="td-mono">DEC-2026-0002</td><td class="td-bold">Mensual</td><td>Enero</td><td>2026</td>
     <td class="td-bold">$4,850.00</td><td>$3,200.00</td>
    ...
    <tr data-tipo="DIOT" data-estatus="presentada">
     <td class="td-mono">DIOT-2026-0002</td><td class="td-bold">DIOT</td><td>Enero</td><td>2026</td>
     <td class="td-bold">—</td><td>—</td><td style="color:rgba(255,255,255,.35)">N/A</td>
     <td><span class="badge badge-presentada"><i class="fas fa-circle" style="font-size:6px"></i> Presentada</span></td>
     <td>17 Feb 2026</td>
     <td style="text-align:center"><div style="display:flex;gap:5px;justify-content:center"><a href="#" class="act-btn green"><i class="fas fa-eye"></i></a><a href="#" class="act-btn"><i class="fas fa-download"></i></a></div></td>
    </tr>
    <tr data-tipo="Mensual" data-estatus="pendiente">
     <td class="td-mono" style="color:rgba(255,255,255,.3)">Pendiente</td><td class="td-bold">Mensual</td><td>Febrero</td><td>2026</td>
     <td class="td-bold" style="color:rgba(255,255,255,.35)">—</td><td style="color:rgba(255,255,255,.35)">—</td><td style="color:rgba(255,255,255,.35)">—</td>
    ...
    <tr data-tipo="Anual" data-estatus="pendiente">
     <td class="td-mono" style="color:rgba(255,255,255,.3)">Pendiente</td><td class="td-bold">Anual</td><td>Enero – Diciembre</td><td>2025</td>
     <td class="td-bold" style="color:rgba(255,255,255,.35)">—</td><td style="color:rgba(255,255,255,.35)">—</td><td style="color:rgba(255,255,255,.35)">—</td>
     <td><span class="badge badge-pendiente"><i class="fas fa-circle" style="font-size:6px"></i> Pendiente</span></td>
...
<div id="form-declaracion" style="scroll-margin-top:80px">
 ...
 <div class="tipo-tabs">
  <button class="tipo-tab active" id="tab-mensual" onclick="setTipoTab('mensual')"><i class="fas fa-calendar-day" style="margin-right:6px"></i>Declaración Mensual</button>
  <button class="tipo-tab" id="tab-anual" onclick="setTipoTab('anual')"><i class="fas fa-calendar-alt" style="margin-right:6px"></i>Declaración Anual</button>
  <button class="tipo-tab" id="tab-diot" onclick="setTipoTab('diot')"><i class="fas fa-table" style="margin-right:6px"></i>DIOT</button>
  <button class="tipo-tab" id="tab-complementaria" onclick="setTipoTab('complementaria')"><i class="fas fa-copy" style="margin-right:6px"></i>Complementaria</button>
 </div>
   ...
   <form action="{{ route('declaraciones.store') }}" method="POST" id="form-mensual">
    ...
    <div class="form-section">
     ...
     <div class="form-grid-3">
      <div class="form-group"><label class="form-label">RFC <span class="req">*</span></label><input class="form-input" name="rfc" value="{{ Auth::user()->rfc ?? '' }}" readonly></div>
      <div class="form-group"><label class="form-label">Período <span class="req">*</span></label>
       <select class="form-input" name="periodo" required><option value="">Selecciona el mes</option>
        @foreach(['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'] as $mes)
         <option value="{{ $mes }}">{{ $mes }}</option>
        @endforeach
       </select>
      </div>
      <div class="form-group"><label class="form-label">Ejercicio <span class="req">*</span></label>
       <select class="form-input" name="ejercicio" required>
        @for($y = now()->year; $y >= now()->year - 5; $y--)<option value="{{ $y }}" {{ $y == now()->year ? 'selected' : '' }}>{{ $y }}</option>@endfor
       </select>
      </div>
      <div class="form-group"><label class="form-label">Tipo</label>
       <select class="form-input" name="tipo_presentacion"><option value="normal">Normal</option><option value="complementaria">Complementaria</option><option value="extemporanea">Extemporánea</option></select>
      </div>
      <div class="form-group"><label class="form-label">Régimen Fiscal</label><input class="form-input" value="{{ Auth::user()->regimen_fiscal ?? 'Régimen Simplificado de Confianza' }}" readonly></div>
     </div>
    </div>
    <div class="form-section">
     <div class="form-section-title"><i class="fas fa-chart-line" style="color:rgba(255,255,255,.3)"></i> Ingresos del período</div>
     <div class="form-grid-2">
      <div class="form-group"><label class="form-label">Ingresos cobrados del período <span class="req">*</span></label><div class="form-input-wrap"><span class="input-prefix">$</span><input class="form-input has-prefix" type="number" name="ingresos_cobrados" id="m_ing_cobrados" step="0.01" min="0" placeholder="0.00" oninput="calcularMensual()" required></div></div>
      <div class="form-group"><label class="form-label">Ingresos exentos</label><div class="form-input-wrap"><span class="input-prefix">$</span><input class="form-input has-prefix" type="number" name="ingresos_exentos" id="m_ing_exentos" step="0.01" min="0" placeholder="0.00" oninput="calcularMensual()"></div></div>
      <div class="form-group"><label class="form-label">IVA trasladado cobrado</label><div class="form-input-wrap"><span class="input-prefix">$</span><input class="form-input has-prefix" type="number" name="iva_trasladado" id="m_iva_trasl" step="0.01" min="0" placeholder="0.00" oninput="calcularMensual()"></div></div>
      <div class="form-group"><label class="form-label">IVA acreditable pagado</label><div class="form-input-wrap"><span class="input-prefix">$</span><input class="form-input has-prefix" type="number" name="iva_acreditable" id="m_iva_acred" step="0.01" min="0" placeholder="0.00" oninput="calcularMensual()"></div></div>
     </div>
    </div>
    <div class="form-section">
     <div class="form-section-title"><i class="fas fa-minus-circle" style="color:rgba(255,255,255,.3)"></i> Deducciones autorizadas</div>
     <div class="form-grid-3">
      <div class="form-group"><label class="form-label">Gastos con comprobante</label><div class="form-input-wrap"><span class="input-prefix">$</span><input class="form-input has-prefix" type="number" name="deduc_gastos" id="m_ded_gastos" step="0.01" min="0" placeholder="0.00" oninput="calcularMensual()"></div></div>
      <div class="form-group"><label class="form-label">Nómina pagada</label><div class="form-input-wrap"><span class="input-prefix">$</span><input class="form-input has-prefix" type="number" name="deduc_nomina" step="0.01" min="0" placeholder="0.00" oninput="calcularMensual()"></div></div>
      <div class="form-group"><label class="form-label">Otras deducciones</label><div class="form-input-wrap"><span class="input-prefix">$</span><input class="form-input has-prefix" type="number" name="deduc_otras" step="0.01" min="0" placeholder="0.00" oninput="calcularMensual()"></div></div>
     </div>
    </div>
    <div class="form-section">
     ...
     <div class="form-grid-3">
      <div class="form-group"><label class="form-label">ISR retenido por clientes</label><div class="form-input-wrap"><span class="input-prefix">$</span><input class="form-input has-prefix" type="number" name="isr_retenido" id="m_isr_ret" step="0.01" min="0" placeholder="0.00" oninput="calcularMensual()"></div></div>
      <div class="form-group"><label class="form-label">IVA retenido por clientes</label><div class="form-input-wrap"><span class="input-prefix">$</span><input class="form-input has-prefix" type="number" name="iva_retenido" id="m_iva_ret" step="0.01" min="0" placeholder="0.00" oninput="calcularMensual()"></div></div>
      <div class="form-group"><label class="form-label">Pagos provisionales anteriores</label><div class="form-input-wrap"><span class="input-prefix">$</span><input class="form-input has-prefix" type="number" name="pagos_provisionales" id="m_pag_prov" step="0.01" min="0" placeholder="0.00" oninput="calcularMensual()"></div></div>
     </div>
    </div>
    <div class="form-section">
     <div class="form-section-title"><i class="fas fa-calculator" style="color:rgba(255,255,255,.3)"></i> Resumen de ISR / IVA</div>
     <div class="isr-resumen"><div class="isr-grid">
      <div class="isr-col"><div class="isr-label">ISR Determinado</div><div class="isr-value" id="m_isr_det">$0.00</div><input type="hidden" name="isr_determinado" id="m_isr_det_h" value="0"></div>
      <div class="isr-col" style="text-align:center"><div class="isr-label">IVA a pagar</div><div class="isr-value" id="m_iva_pagar">$0.00</div><input type="hidden" name="iva_pagar" id="m_iva_pagar_h" value="0"></div>
      <div class="isr-col-saldo" style="border-right:none"><div class="isr-label">Saldo a cargo / favor</div><div class="isr-value positive" id="m_saldo">$0.00</div><input type="hidden" name="saldo" id="m_saldo_h" value="0"></div>
     </div></div>
    </div>
    <div class="protest-wrap"><input type="checkbox" name="protesta" id="m_protesta" required><label for="m_protesta" class="protest-text"><strong>Bajo protesta de decir verdad</strong>, declaro que los datos manifestados en esta declaración son correctos y que representan fielmente mi situación fiscal del período declarado. <span class="req">*</span></label></div>
    <div class="form-footer">
     <div class="footer-deadline"><i class="fas fa-calendar-alt"></i>Fecha límite: <span>17 de {{ now()->format('F Y') }}</span></div>
 ...
 <div id="form-anual-wrap" style="display:none">
  <div class="form-hero">
   ...
   <div class="form-hero-info"><div class="form-hero-title">Declaración Anual</div><div class="form-hero-sub">Ejercicio fiscal {{ now()->year - 1 }}</div></div>
  ...
  <div class="panel-card">
   <form action="{{ route('declaraciones.store') }}" method="POST" id="form-anual">
    ...
    <div class="form-section">
     <div class="form-section-title"><i class="fas fa-id-card" style="color:rgba(255,255,255,.3)"></i> Identificación</div>
     <div class="form-grid-3">
      <div class="form-group"><label class="form-label">RFC <span class="req">*</span></label><input class="form-input" value="{{ Auth::user()->rfc ?? '' }}" readonly></div>
      <div class="form-group"><label class="form-label">Ejercicio fiscal <span class="req">*</span></label>
       <select class="form-input" name="ejercicio" required>@for($y = now()->year - 1; $y >= now()->year - 6; $y--)<option value="{{ $y }}" {{ $y == now()->year - 1 ? 'selected' : '' }}>{{ $y }}</option>@endfor</select>
      </div>
      <div class="form-group"><label class="form-label">Tipo de declaración</label>
       <select class="form-input" name="tipo_presentacion_anual"><option value="normal">Normal</option><option value="complementaria">Complementaria</option><option value="extemporanea">Extemporánea</option></select>
      </div>
     </div>
    </div>
    <div class="form-section">
     ...
     <div class="form-grid-2">
      <div class="form-group"><label class="form-label">Total de ingresos acumulables <span class="req">*</span></label><div class="form-input-wrap"><span class="input-prefix">$</span><input class="form-input has-prefix" type="number" name="ingresos_acumulables" id="a_ing_acum" step="0.01" min="0" placeholder="0.00" oninput="calcularAnual()" required></div></div>
      <div class="form-group"><label class="form-label">Ingresos exentos</label><div class="form-input-wrap"><span class="input-prefix">$</span><input class="form-input has-prefix" type="number" name="ingresos_exentos_anual" id="a_ing_exent" step="0.01" min="0" placeholder="0.00" oninput="calcularAnual()"></div></div>
     </div>
    </div>
    <div class="form-section">
     <div class="form-section-title"><i class="fas fa-minus-circle" style="color:rgba(255,255,255,.3)"></i> Deducciones autorizadas</div>
     <div class="form-grid-3">
      <div class="form-group"><label class="form-label">Honorarios médicos</label><div class="form-input-wrap"><span class="input-prefix">$</span><input class="form-input has-prefix" type="number" name="ded_honorarios_medicos" step="0.01" min="0" placeholder="0.00" oninput="calcularAnual()"></div></div>
      <div class="form-group"><label class="form-label">Gastos hospitalarios</label><div class="form-input-wrap"><span class="input-prefix">$</span><input class="form-input has-prefix" type="number" name="ded_gastos_hospitalarios" step="0.01" min="0" placeholder="0.00" oninput="calcularAnual()"></div></div>
      <div class="form-group"><label class="form-label">Primas de seguro médico</label><div class="form-input-wrap"><span class="input-prefix">$</span><input class="form-input has-prefix" type="number" name="ded_seguro_medico" step="0.01" min="0" placeholder="0.00" oninput="calcularAnual()"></div></div>
      <div class="form-group"><label class="form-label">Colegiaturas</label><div class="form-input-wrap"><span class="input-prefix">$</span><input class="form-input has-prefix" type="number" name="ded_colegiaturas" step="0.01" min="0" placeholder="0.00" oninput="calcularAnual()"></div></div>
      <div class="form-group"><label class="form-label">Intereses crédito hipotecario</label><div class="form-input-wrap"><span class="input-prefix">$</span><input class="form-input has-prefix" type="number" name="ded_intereses_hipotecarios" step="0.01" min="0" placeholder="0.00" oninput="calcularAnual()"></div></div>
      <div class="form-group"><label class="form-label">Donativos</label><div class="form-input-wrap"><span class="input-prefix">$</span><input class="form-input has-prefix" type="number" name="ded_donativos" step="0.01" min="0" placeholder="0.00" oninput="calcularAnual()"></div></div>
      <div class="form-group"><label class="form-label">Aportaciones a AFORE</label><div class="form-input-wrap"><span class="input-prefix">$</span><input class="form-input has-prefix" type="number" name="ded_afore" step="0.01" min="0" placeholder="0.00" oninput="calcularAnual()"></div></div>
      <div class="form-group"><label class="form-label">Transporte escolar</label><div class="form-input-wrap"><span class="input-prefix">$</span><input class="form-input has-prefix" type="number" name="ded_transporte_escolar" step="0.01" min="0" placeholder="0.00" oninput="calcularAnual()"></div></div>
      <div class="form-group"><label class="form-label">Otras deducciones</label><div class="form-input-wrap"><span class="input-prefix">$</span><input class="form-input has-prefix" type="number" name="ded_otras_anual" step="0.01" min="0" placeholder="0.00" oninput="calcularAnual()"></div></div>
     </div>
    </div>
    <div class="form-section">
     ...
     <div class="form-grid-3">
      <div class="form-group"><label class="form-label">ISR retenido total del ejercicio</label><div class="form-input-wrap"><span class="input-prefix">$</span><input class="form-input has-prefix" type="number" name="isr_retenido_anual" id="a_isr_ret" step="0.01" min="0" placeholder="0.00" oninput="calcularAnual()"></div></div>
      <div class="form-group"><label class="form-label">Pagos provisionales anuales</label><div class="form-input-wrap"><span class="input-prefix">$</span><input class="form-input has-prefix" type="number" name="pagos_provisionales_anual" id="a_pag_prov" step="0.01" min="0" placeholder="0.00" oninput="calcularAnual()"></div></div>
      <div class="form-group"><label class="form-label">Estímulos fiscales</label><div class="form-input-wrap"><span class="input-prefix">$</span><input class="form-input has-prefix" type="number" name="estimulos_fiscales" step="0.01" min="0" placeholder="0.00" oninput="calcularAnual()"></div></div>
     </div>
    </div>
    <div class="form-section">
     <div class="form-section-title"><i class="fas fa-calculator" style="color:rgba(255,255,255,.3)"></i> Resumen de ISR</div>
     <div class="isr-resumen"><div class="isr-grid">
      <div class="isr-col"><div class="isr-label">ISR Determinado</div><div class="isr-value" id="a_isr_det">$0.00</div><input type="hidden" name="isr_determinado" id="a_isr_det_h" value="0"></div>
      <div class="isr-col" style="text-align:center"><div class="isr-label" style="margin-bottom:10px">ISR Retenido (manual)</div><input class="isr-input" type="number" name="isr_retenido_input" id="a_isr_ret_inp" step="0.01" min="0" placeholder="0.00" oninput="calcularAnual()"><input type="hidden" name="isr_retenido" id="a_isr_ret_h" value="0"></div>
      <div class="isr-col-saldo" style="border-right:none"><div class="isr-label">Saldo a cargo / favor</div><div class="isr-value positive" id="a_saldo">$0.00</div><input type="hidden" name="saldo" id="a_saldo_h" value="0"></div>
     </div></div>
    </div>
    <div class="protest-wrap"><input type="checkbox" name="protesta_anual" id="a_protesta" required><label for="a_protesta" class="protest-text"><strong>Bajo protesta de decir verdad</strong>, declaro que los datos manifestados son correctos y que representan fielmente mi situación fiscal del ejercicio. <span class="req">*</span></label></div>
    <div class="form-footer">
     <div class="footer-deadline"><i class="fas fa-calendar-alt"></i>Fecha límite: <span>30 de Abril {{ now()->year }}</span></div>
     <div class="footer-btns">
 ...
 <div id="form-diot-wrap" style="display:none">
  <div class="form-hero" style="background:linear-gradient(135deg,#1e3a5f 0%,#1a2f50 100%)">
   ...
   <div class="form-hero-info"><div class="form-hero-title">DIOT</div><div class="form-hero-sub">Declaración Informativa de Operaciones con Terceros</div></div>
   ...
   <form action="{{ route('declaraciones.store') }}" method="POST">
    ...
    <div class="form-section">
     ...
     <div class="form-grid-3">
      <div class="form-group"><label class="form-label">RFC <span class="req">*</span></label><input class="form-input" value="{{ Auth::user()->rfc ?? '' }}" readonly></div>
      <div class="form-group"><label class="form-label">Período <span class="req">*</span></label>
       <select class="form-input" name="periodo_diot" required><option value="">Selecciona el mes</option>
        @foreach(['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'] as $mes)<option value="{{ $mes }}">{{ $mes }}</option>@endforeach
       </select>
      </div>
      <div class="form-group"><label class="form-label">Ejercicio <span class="req">*</span></label>
       <select class="form-input" name="ejercicio_diot" required>@for($y = now()->year; $y >= now()->year - 5; $y--)<option value="{{ $y }}" {{ $y == now()->year ? 'selected' : '' }}>{{ $y }}</option>@endfor</select>
      </div>
     </div>
    </div>
    <div class="form-section">
     ...
     <div class="form-grid-2">
      <div class="form-group"><label class="form-label">IVA pagado a proveedores nacionales</label><div class="form-input-wrap"><span class="input-prefix">$</span><input class="form-input has-prefix" type="number" name="iva_proveedores_nac" step="0.01" min="0" placeholder="0.00"></div></div>
      <div class="form-group"><label class="form-label">IVA pagado a proveedores extranjeros</label><div class="form-input-wrap"><span class="input-prefix">$</span><input class="form-input has-prefix" type="number" name="iva_proveedores_ext" step="0.01" min="0" placeholder="0.00"></div></div>
      <div class="form-group"><label class="form-label">IVA pagado en importaciones</label><div class="form-input-wrap"><span class="input-prefix">$</span><input class="form-input has-prefix" type="number" name="iva_importaciones" step="0.01" min="0" placeholder="0.00"></div></div>
      <div class="form-group"><label class="form-label">Total de operaciones con terceros</label><div class="form-input-wrap"><span class="input-prefix">$</span><input class="form-input has-prefix" type="number" name="total_operaciones" step="0.01" min="0" placeholder="0.00"></div></div>
     </div>
    ...
    <div class="protest-wrap"><input type="checkbox" name="protesta_diot" id="d_protesta" required><label for="d_protesta" class="protest-text"><strong>Bajo protesta de decir verdad</strong>, declaro que los datos son correctos. <span class="req">*</span></label></div>
    <div class="form-footer">
     <div class="footer-deadline"><i class="fas fa-calendar-alt"></i>Fecha límite: <span>17 del mes siguiente</span></div>
     <div class="footer-btns"><button type="submit" class="btn btn-primary"><i class="fas fa-paper-plane"></i> Presentar DIOT</button></div>
    </div>
   </form>
  </div>
 </div>
 ...
 <div id="form-complementaria-wrap" style="display:none">
  <div class="form-hero" style="background:linear-gradient(135deg,#7c2d12 0%,#6b2510 100%)">
   ...
   <div class="form-hero-info"><div class="form-hero-title">Declaración Complementaria</div><div class="form-hero-sub">Corrección de una declaración previamente presentada</div></div>
  </div>
  <div class="panel-card">
   <form action="{{ route('declaraciones.store') }}" method="POST">
    @csrf
    ...
    <div class="form-section">
     ...
     <div class="form-grid-3">
      <div class="form-group"><label class="form-label">Folio / número de operación <span class="req">*</span></label><input class="form-input" name="folio_original" placeholder="Ej. DEC-2026-0001" required></div>
      <div class="form-group"><label class="form-label">Tipo de declaración original</label>
       <select class="form-input" name="tipo_original"><option value="mensual">Mensual</option><option value="anual">Anual</option><option value="diot">DIOT</option></select>
      </div>
      <div class="form-group"><label class="form-label">Motivo de corrección</label>
       <select class="form-input" name="motivo_correccion"><option value="error_datos">Error en datos</option><option value="omision_ingresos">Omisión de ingresos</option><option value="deduccion_incorrecta">Deducción incorrecta</option><option value="otro">Otro</option></select>
      </div>
      <div class="form-group" style="grid-column:1/-1"><label class="form-label">Descripción de la corrección</label><textarea class="form-input" name="descripcion_correccion" rows="3" placeholder="Describe brevemente qué se está corrigiendo..." style="resize:vertical"></textarea></div>
     </div>
    </div>
    ...
    <div class="protest-wrap"><input type="checkbox" name="protesta_comp" id="c_protesta" required><label for="c_protesta" class="protest-text"><strong>Bajo protesta de decir verdad</strong>, declaro que los datos corregidos son correctos. <span class="req">*</span></label></div>
    <div class="form-footer">
     <div class="footer-deadline"><i class="fas fa-info-circle"></i>Las complementarias no tienen fecha límite, pero se recomienda presentarlas a la brevedad.</div>
     <div class="footer-btns"><button type="submit" class="btn btn-primary"><i class="fas fa-paper-plane"></i> Presentar Complementaria</button></div>
    </div>
   </form>
  </div>
 </div>
...
function calcularAnual(){
 ...
 let dedTotal = 0;
 document.querySelectorAll('#form-anual input[name^="ded_"]').forEach(el=>{ dedTotal += parseFloat(el.value)||0; });
 const base   = Math.max(0, ing - exen - dedTotal);
 const isrDet = base * 0.15;
