...
.reg-progress{
    ...
    padding:20px 40px;
    background:#f8fafc;
    border-bottom:1px solid var(--sat-gray-border);
}
.rp-step{
    display:flex;
...
/* La línea conectora: posicionada en el centro del número, entre pasos */
.rp-step:not(:last-child)::after{
    content:'';
    position:absolute;
    /* Empieza justo a la derecha del círculo (17px = radio del círculo) */
    left:calc(50% + 17px);
    /* Termina en el círculo del siguiente paso */
    right:calc(-50% + 17px);
    top:17px; /* centro vertical del círculo de 34px */
    height:2px;
    background:var(--sat-gray-border);
...
.rp-num{
    ...
    border:2px solid var(--sat-gray-border);
    background:white;color:var(--sat-text-muted);
...
.rp-lbl{
    font-size:11px;font-weight:600;color:var(--sat-text-muted);
...
/* ── RFC FOUND BANNER ──────────────────────────────────────────── */
.rfc-found-banner{
    background:#f0f9f4;border:1.5px solid var(--sat-green);border-radius:10px;
    padding:14px 18px;margin-top:14px;display:none;align-items:center;gap:14px;
}
.rfc-found-banner.show{display:flex}
.rfc-found-banner .rfb-icon{
    width:36px;height:36px;background:var(--sat-green);border-radius:50%;
    display:flex;align-items:center;justify-content:center;color:white;
    font-size:15px;flex-shrink:0;
}
.rfc-found-banner .rfb-text{flex:1;font-size:13px;color:var(--sat-dark)}
.rfc-found-banner .rfb-text strong{color:var(--sat-green)}
.rfc-found-banner .rfb-badge{
    background:var(--sat-green);color:white;font-size:11px;font-weight:700;
...
.rfc-checking{
    ...
    font-size:12px;color:var(--sat-text-muted);margin-top:6px;
}
.rfc-checking.show{display:flex}
...
@media(max-width:640px){
    .registro-header,.reg-body,.reg-footer{padding-left:20px;padding-right:20px}
    .reg-progress{padding:16px 20px}
    .res-grid,.tipo-grid{grid-template-columns:1fr}
...
<div class="registro-page">
 <div class="registro-card">
  ...
  <div class="reg-progress" id="reg-prog">
   <div class="rp-step active" id="rp-1">
    <div class="rp-num">1</div>
    <div class="rp-lbl">Tipo</div>
   </div>
   <div class="rp-step" id="rp-2">
    <div class="rp-num">2</div>
    <div class="rp-lbl">Datos</div>
   </div>
   <div class="rp-step" id="rp-3">
    <div class="rp-num">3</div>
    <div class="rp-lbl">Acceso</div>
   </div>
   <div class="rp-step" id="rp-4">
    <div class="rp-num"><i class="fas fa-check" style="font-size:11px"></i></div>
    <div class="rp-lbl">Confirmar</div>
   </div>
  </div>
  ...
  <form action="{{ route('registro.post') }}" method="POST" id="regForm">
     ...
     <div id="campos-fisica">
      <div class="sat-form-row cols-3">
       <div class="sat-form-group">
        ...
        <div class="fi-wrap"><i class="fas fa-user fi"></i>
         <input type="text" name="primer_apellido" id="inp-primer-apellido" class="sat-input" placeholder="Apellido paterno" maxlength="50">
        </div>
       ...
       <div class="sat-form-group">
        ...
        <div class="fi-wrap"><i class="fas fa-user fi"></i>
         <input type="text" name="segundo_apellido" id="inp-segundo-apellido" class="sat-input" placeholder="Apellido materno" maxlength="50">
        </div>
       </div>
       <div class="sat-form-group">
        <label>Nombre(s) <span class="required">*</span></label>
        <div class="fi-wrap"><i class="fas fa-signature fi"></i>
         <input type="text" name="nombres" id="inp-nombres" class="sat-input" placeholder="Nombre(s)" maxlength="80">
        </div>
        <span class="sat-input-error">Campo requerido</span>
       </div>
      </div>
      <div class="sat-form-row cols-2">
       <div class="sat-form-group">
        ...
        <div class="fi-wrap"><i class="fas fa-id-card fi"></i>
         <input type="text" name="curp" id="curp-reg" class="sat-input" placeholder="18 caracteres" maxlength="18" style="text-transform:uppercase;font-family:monospace;letter-spacing:1px">
        </div>
       ...
       <div class="sat-form-group">
        ...
        <div class="fi-wrap"><i class="fas fa-birthday-cake fi"></i>
         <input type="date" name="fecha_nacimiento" id="inp-fecha-nacimiento" class="sat-input" max="{{ date('Y-m-d') }}">
        </div>
     ...
     <div id="campos-moral" style="display:none">
      <div class="sat-form-row cols-2">
       <div class="sat-form-group">
        ...
        <div class="fi-wrap"><i class="fas fa-building fi"></i>
         <input type="text" name="razon_social" class="sat-input" placeholder="Nombre oficial de la empresa" maxlength="200">
        </div>
       </div>
       <div class="sat-form-group">
      ...
      <div class="sat-form-row cols-2">
       <div class="sat-form-group">
        ...
        <div class="fi-wrap"><i class="fas fa-user-tie fi"></i>
         <input type="text" name="rep_nombre" class="sat-input" placeholder="Nombre completo" maxlength="150">
        </div>
       </div>
       <div class="sat-form-group">
        <label>RFC del representante <span class="required">*</span></label>
        <div class="fi-wrap"><i class="fas fa-hashtag fi"></i>
         <input type="text" name="rep_rfc" class="sat-input" placeholder="13 caracteres" maxlength="13" style="text-transform:uppercase;font-family:monospace">
        </div>
       </div>
      </div>
     </div>

     {{-- RFC + Teléfono (comunes a ambos tipos) --}}
     <div class="sat-form-row cols-2">
      <div class="sat-form-group">
       <label>RFC <span class="required">*</span></label>
       <div class="fi-wrap">
        <i class="fas fa-hashtag fi"></i>
        <input type="text" name="rfc" id="rfc-reg" class="sat-input"
               placeholder="Ej: GOML850101ABC" maxlength="13"
               style="text-transform:uppercase;font-family:monospace;letter-spacing:1px"
               autocomplete="off">
       </div>
       {{-- Spinner de búsqueda --}}
       <div class="rfc-checking" id="rfc-checking">
        <i class="fas fa-spinner fa-spin" style="color:var(--sat-green)"></i>
        <span>Buscando RFC en el sistema...</span>
       </div>
       <span class="sat-input-error">RFC inválido</span>
      </div>
      <div class="sat-form-group">
       <label>Teléfono celular <span class="required">*</span></label>
       <div class="fi-wrap"><i class="fas fa-mobile-alt fi"></i>
        <input type="tel" name="telefono" id="inp-telefono" class="sat-input" placeholder="10 dígitos" maxlength="10">
       </div>
       <span class="sat-input-error">Teléfono requerido</span>
      </div>
     </div>

     {{-- Banner: RFC encontrado en el sistema --}}
     <div class="rfc-found-banner" id="rfc-found-banner">
      <div class="rfb-icon"><i class="fas fa-check"></i></div>
      <div class="rfb-text">
       RFC encontrado — <strong id="rfb-nombre">—</strong><br>
       <span style="font-size:12px;color:var(--sat-text-muted)">Los datos han sido pre-llenados desde el sistema. Puedes editarlos si es necesario.</span>
      </div>
      <div class="rfb-badge"><i class="fas fa-database" style="margin-right:4px"></i>Del sistema</div>
     </div>

     {{-- Preview del RFC ingresado --}}
     <div class="rfc-box" id="rfc-preview">
      <i class="fas fa-id-badge" style="color:var(--sat-green);font-size:20px"></i>
      <div style="flex:1">
       <div style="font-size:11px;color:var(--sat-text-muted);margin-bottom:2px">RFC ingresado</div>
       <div class="rfc-val" id="rfc-val-text">—</div>
      </div>
    ...
    <div class="reg-step" id="step-3">
     ...
     <div class="sat-form-row cols-2">
      <div class="sat-form-group">
       ...
       <div class="fi-wrap"><i class="fas fa-envelope fi"></i>
        <input type="email" name="email" id="email-reg" class="sat-input" placeholder="correo@ejemplo.com">
       </div>
      ...
      <div class="sat-form-group">
       ...
       <div class="fi-wrap"><i class="fas fa-envelope fi"></i>
        <input type="email" name="email_confirmation" id="email-c-reg" class="sat-input" placeholder="correo@ejemplo.com" autocomplete="off">
       </div>
     ...
     <div class="sat-form-row cols-2">
      <div class="sat-form-group">
       ...
       <div class="eye-wrap">
        <input type="password" name="password" id="pwd-reg" class="sat-input" placeholder="Mínimo 8 caracteres">
        <button type="button" class="eye-btn" data-target="pwd-reg"><i class="fas fa-eye"></i></button>
       </div>
      ...
      <div class="sat-form-group">
       ...
       <div class="eye-wrap">
        <input type="password" name="password_confirmation" id="pwd-c" class="sat-input" placeholder="Repite la contraseña">
        <button type="button" class="eye-btn" data-target="pwd-c"><i class="fas fa-eye"></i></button>
       </div>
   ...
   <div class="reg-footer">
    ...
    <div style="display:flex;gap:12px">
     <button type="button" class="btn-sat-outline" id="btn-prev" style="display:none" onclick="prevStep()">
      <i class="fas fa-arrow-left"></i> Anterior
     </button>
     <button type="button" class="btn-sat-green" id="btn-next" onclick="nextStep()">
      Continuar <i class="fas fa-arrow-right"></i>
     </button>
     <button type="submit" class="btn-sat-green" id="btn-submit" style="display:none">
...
let rfcTimer = null; // debounce para la búsqueda AJAX
...
function goTo(n) {
    ...
    for (let i = 1; i <= 4; i++) {
        ...
        if (i < 4) rp.querySelector('.rp-num').innerHTML = i < n
...
function validate(s) {
    ...
    if (s === 2) {
        ...
        if (!/^[A-ZÑ&]{3,4}\d{6}[A-Z0-9]{3}$/i.test(rfc)) {
            showErr(document.getElementById('rfc-reg'), 'RFC inválido'); return false;
        }
        if (tipo === 'fisica') {
...
function fillResumen() {
    ...
    const nom = tipo === 'fisica'
        ? [g('primer_apellido'), g('segundo_apellido'), g('nombres')].filter(x => x !== '—').join(' ')
        : g('razon_social');
    ...
    if (tipo === 'fisica') {
        document.getElementById('r-curp').textContent = g('curp').toUpperCase();
        cb.style.display = '';
    } else {
...
// ── AUTO-FILL: Buscar RFC en solicitudes_rfc ──────────────────────────────
async function buscarRfc(rfc) {
    const banner   = document.getElementById('rfc-found-banner');
    const checking = document.getElementById('rfc-checking');

    banner.classList.remove('show');
    checking.classList.add('show');

    try {
        const res  = await fetch(`/personas/rfc/buscar/${encodeURIComponent(rfc)}`, {
            headers: { 'X-Requested-With': 'XMLHttpRequest' }
        });
        checking.classList.remove('show');

        if (!res.ok) return; // 404 = no encontrado, sin mensaje

        const data = await res.json();

        // Llenar campos persona física
        if (data.primer_apellido)   document.getElementById('inp-primer-apellido').value   = data.primer_apellido;
        if (data.segundo_apellido)  document.getElementById('inp-segundo-apellido').value  = data.segundo_apellido;
        if (data.nombres)           document.getElementById('inp-nombres').value           = data.nombres;
        if (data.curp)              document.getElementById('curp-reg').value              = data.curp.toUpperCase();
        if (data.fecha_nacimiento)  document.getElementById('inp-fecha-nacimiento').value  = data.fecha_nacimiento;
        if (data.telefono)          document.getElementById('inp-telefono').value          = data.telefono;

        // Mostrar banner de confirmación
        const nombre = [data.nombres, data.primer_apellido, data.segundo_apellido].filter(Boolean).join(' ');
        document.getElementById('rfb-nombre').textContent = nombre || rfc;
...
// ── Input RFC: formato + preview + auto-fill (debounced) ──────────────────
document.getElementById('rfc-reg')?.addEventListener('input', function () {
    ...
    if (ok) {
        ...
        document.getElementById('rfc-val-text').textContent = this.value;
        document.getElementById('rfc-tipo-lbl').textContent = this.value.length === 12 ? 'Persona Moral' : 'Persona Física';
        // Debounce: esperar 600ms después de que el usuario deje de escribir
        clearTimeout(rfcTimer);
        rfcTimer = setTimeout(() => buscarRfc(this.value), 600);
    } else {
        box.classList.remove('show');
        document.getElementById('rfc-found-banner').classList.remove('show');
        document.getElementById('rfc-checking').classList.remove('show');
        clearTimeout(rfcTimer);
    }
...
// ── CURP: solo mayúsculas y alfanumérico ─────────────────────────────────
document.getElementById('curp-reg')?.addEventListener('input', function () {
...
document.querySelectorAll('.eye-btn').forEach(b => b.addEventListener('click', function () {
    ...
    ic.classList.toggle('fa-eye',       i.type === 'password');
    ic.classList.toggle('fa-eye-slash', i.type === 'text');
}));

// ── Limpiar error al escribir ─────────────────────────────────────────────
document.querySelectorAll('.sat-input,.sat-select').forEach(el =>
